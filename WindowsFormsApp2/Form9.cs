...
namespace WindowsFormsApp2
{
    public partial class Form9 : Form
    {
        ...
        public Form9()
        {                                     //tüm çıkacak butonları görünmez kıldık en başta
            InitializeComponent();
        ...
        private void oyunBitti(string message) //skorun yollandığı
        {
            zamanlayıcı.Stop();
            ...
            if (oku.Read())
            {
                ...
                {
                    OleDbCommand komut = new OleDbCommand("update aaaaa set oyun6='" + skor + "' where kullaniciadi='" + Form1.kulad + "'", baglanti); 
        ...
        private void oyunZamanlayıcı(object sender, EventArgs e) //oyuncu hareketi
        {
            ...
            top.Left += topx;// yatay hareket
            top.Top += topy; //dikey hareket
            ...
            foreach (Control x in this.Controls)
            {
                if (x is PictureBox && (string)x.Tag == "bloklar")    //bloklara çarptıgında skor 1 artar 
                {
                    if (top.Bounds.IntersectsWith(x.Bounds))
                    {
                        skor += 1;

                        topy = - topy; //top yönü değşir çarpma sonrası
